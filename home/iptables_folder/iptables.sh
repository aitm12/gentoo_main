...
# outbound dns
iptables -A OUTPUT -o eno1 -p udp --dport 53 -m state --state NEW,ESTABLISHED -j ACCEPT
iptables -A INPUT -i eno1 -p udp --dport -m state --state ESTABLISHED -j ACCEPT
...
iptables pA INPUT -i eno1 -p tcp -d rsync.gentoo.org --sport 873 -m state --state ESTABLISHED -j ACCEPT
...
iptables -o OUTPUT -o eno1 -p tcp -d gentoo.cs.utah.edu --dport 80 -m state --state NEW,ESTABLISHED -j ACCEPT

# outbound HTTP
iptables -A OUTPUT -o eno1 -p tcp --dport 80 -m state --state NEW,ESTABLISHED -j ACCEPT
iptables -A INPUT -i eno1 -p tcp --sport 80 -m limit --limit 25/minute --limit-burst 100 state --state ESTABLISHED -j ACCEPT
...
iptables -A INOUT -i eno1 -p tcp --sport 993 -m state --state ESTABLISHED -j ACCEPT
